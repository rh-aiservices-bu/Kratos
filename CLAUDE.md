...
├── docs/
│   ├── architecture/
│   │   ├── adrs/                          # Architecture Decision Records (ADR-001 to ADR-020)
...
10. **Scenario categories (ADR-020)**: open the scenario list in the UI → confirm 5 populated category sections (Load Testing, Rate Limiting, Access Control, Metrics Validation, API Key Lifecycle) in that order, plus a visible, empty "Custom" section with its placeholder text → drop a scenario YAML with no `category:` field into `scenarios/` and confirm it appears under Custom without any code change.
