...
│       ├── platform_health.py  # check_platform_health (ADR-022): read-only TokenRateLimitPolicy/Gateway/HTTPRoute health checks
...
│   ├── metrics_fill.yaml
│   ├── access_denied_no_policy.yaml
│   ├── api_key_lifecycle.yaml
│   ├── rate_limit_priority_precedence.yaml
│   ├── platform_health_check.yaml
│   └── rate_limit_shared_across_users.yaml  # ADR-023: two ServiceAccount-minted users, one shared MaaSSubscription
│
├── api/                        # FastAPI backend
│   ├── __init__.py
...
- `get` on `pods/log`
- `get`, `create`, `patch`, `delete` on `maassubscriptions` and `maasauthpolicies` (`maas.opendatahub.io/v1alpha1`) — for `rate_limit_validation` and `access_denied_no_policy` scenarios respectively (ADR-018)
...
### Scenario Categories (ADR-020)

`ScenarioList.tsx` groups scenarios by a `category:` field (optional in the YAML, defaulted to `"Custom"` by `api/routes/scenarios.py` when absent) into a fixed set of sections: Load Testing, Rate Limiting, Access Control, Metrics Validation, API Key Lifecycle, Platform Health, and an always-rendered Custom bucket (shown even when empty, with a short empty-state message, so the "write your own scenario" path is discoverable). A category string that isn't one of the 6 known built-ins also falls into Custom. The category order lives in two places kept in sync by hand — `CATEGORY_ORDER` in `ui/src/components/ScenarioList.tsx` and `_KNOWN_CATEGORIES` in `harness/tests/test_scenarios.py` (the latter fails a build if a scenario file declares an unrecognized category).
...
Every scenario that sends inference traffic routes its `send_requests`-family assertions through the `promql`-form (ADR-015) rather than the plain string form — even where the check is a pure harness-side number (`error_rate_pct`, `p99_latency_ms`, `throughput_rps`) with no real backing Prometheus metric, the value is passed through as a `${harness.x}`-substituted literal (e.g. `promql: "${harness.inference_results.error_rate_pct}"`, `expect: "< 5"`), so every such scenario run exercises the real query-firing/parsing path against the live Thanos Querier, not just `metrics_fill`. This was a deliberate choice to broaden test coverage of the new mechanism, not a claim that these checks became more meaningful — `direct_inference` in particular bypasses the MaaS gateway entirely, so there's no real MaaS-side metric to check there regardless of form. (`platform_health_check` is the one exception across every scenario — it never sends inference at all, so its assertions are on `check_platform_health`'s own read-only flags instead.)

| Scenario (Category) | Tasks | Default Config | Default Assertions | Notes |
|---|---|---|---|---|
| `single_key_load` (Load Testing) | provision_api_key → send_requests → [cleanup: delete key] | `request_count: 100`, `concurrency: 5` | `error_rate_pct`/`p99_latency_ms` — promql pass-through, `< 5`/`< 10000` | Baseline load test — one key, N requests through MaaS. MaaS metrics polled in background automatically. |
| `multi_key_load` (Load Testing) | provision_api_key × N → send_requests (M reqs distributed across key pool) → [cleanup: delete all N keys] | `key_count: 5`, `request_count: 5`, `concurrency: 5` | `error_rate_pct` — promql pass-through, `< 5` | M requests distributed evenly across N keys (floor(M/N) per key, remainder to first). Default M=N so each key gets exactly 1 probe. |
| `direct_inference` (Load Testing) | send_requests (url=target_url, token=target_token) — no key provisioning, no cleanup | `target_url: ""` (required), `target_token: ""` (required), `request_count: 50`, `concurrency: 5` | `error_rate_pct` — promql pass-through, `< 5` | Send inference directly to any configurable endpoint. Bypasses MaaS gateway — tests underlying model serving or compares MaaS-routed vs direct latency. No real MaaS-side metric applies here regardless of assertion form. |
| `rate_limit_validation` (Rate Limiting) | apply_rate_limit_subscription(rps_limit) → provision_api_key × N → send_requests → [cleanup: delete keys + restore/delete MaaSSubscription] | `rate_limit_rps: 10`, `request_count: 100`, `concurrency: 20`, `key_count: 1` | `throughput_rps`/`error_rate_pct` — promql pass-through, `<= ${config.rate_limit_rps}`/`< 30`; `maas_denied_by_rate_limit` (ADR-018) — baseline-delta `promql` check on Limitador's real `limited_calls`, `> 0`; `subscription_echo_match_count` (ADR-019) — `== key_count` | Creates a `MaaSSubscription` CR with a configured rate limit. Fires requests and asserts observed throughput stays at or below the limit, **and** that Limitador itself actually recorded a denial — not just that the harness's own throughput math stayed under budget. Some 429s are expected. |
| `metrics_fill` (Metrics Validation) | provision_api_key → send_requests → [cleanup: delete key] | `request_count: 200`, `concurrency: 10`, `metrics_tolerance_pct: 5` | `error_rate_pct` — promql pass-through, `< 5`; `maas_requests_match`/`maas_tokens_match` — self-contained `promql`-form assertions (ADR-015) asserting MaaS-reported request/token deltas match `inference_results.total_requests`/`total_tokens_sent` within tolerance | The canonical MaaS-vs-harness cross-check scenario (ADR-014/ADR-015) — sends a burst and asserts MaaS-reported request/token counts actually match what the harness sent, not just that they're non-zero. |
| `access_denied_no_policy` (Access Control, ADR-018) | apply_rate_limit_subscription (synthetic group, generous limit) → provision_api_key (pinned) → send_requests → [cleanup: delete key + restore/delete MaaSSubscription] | `test_group: "maaspal-fail-closed-test"`, `token_limit: 1000000`, `request_count: 20`, `concurrency: 5` | `error_rate_pct` — promql pass-through, `> 90`; `rate_limited_count`/`unauthorized_count` — promql pass-through, `== 0`/`> 0`; `subscription_echo_match_count` (ADR-019) — `== key_count` | Grants a synthetic group quota via a `MaaSSubscription` but deliberately leaves no matching `MaaSAuthPolicy` for it, then asserts the gateway actually fails closed (`gateway-default-deny`) — the empirical check behind the MaaS Setup UI's Access Control tab and Access Simulator. Does **not** create a `MaaSAuthPolicy` (that's the point); `apply_auth_policy` exists for a future scenario testing the inverse case, see `docs/architecture/empirical-verification-checklist.md`. |
| `api_key_lifecycle` (API Key Lifecycle, ADR-019) | provision_api_key → verify_api_key_search → send_requests → revoke_api_keys → verify_revoked_key_denied → [cleanup: delete keys (already revoked, harmless no-op)] | `key_name: "maaspal-lifecycle-key"`, `key_count: 3`, `request_count: 10`, `concurrency: 3` | `name_echo_match_count`/`expires_at_present_count` — `== key_count`; `found_count` — `== key_count`; `error_rate_pct` (before revoke) — `< 5`; `error_rate_pct`/`unauthorized_count` (after revoke) — `> 90`/`> 0` | The first MaaS:PAL scenario with **zero** Kubernetes CR dependency — everything goes through `/maas-api/v1/api-keys*`. Confirms the create response honestly echoes what was requested, `/search` actually finds this run's keys, and a revoked key is denied by the very next request. |
| `rate_limit_priority_precedence` (Rate Limiting, ADR-021) | apply_priority_test_subscriptions (2 subscriptions, same owner) → provision_api_key (unpinned) → send_requests → [cleanup: delete both subscriptions] | `low_priority: 50`, `high_priority: 200`, `low_priority_token_limit: 10`, `high_priority_token_limit: 1000000` | `expected_subscription_match_count` — `== 1`; `error_rate_pct` — promql pass-through, `< 30` | Confirmed live: priority isn't arbitrated per-request — MaaS resolves it once, at key-creation time, deciding which subscription's key a caller gets. Tests that auto-selection (no `subscription` pinned) actually picks the higher-priority one, via the create response's echoed `subscription`. |
| `platform_health_check` (Platform Health, ADR-022) | check_platform_health — read-only, no key provisioning, no traffic, no cleanup | `gateway_name: "maas-default-gateway"`, `gateway_namespace: "openshift-ingress"` | `rate_limit_policy_accepted`/`_enforced`, `gateway_programmed`, `http_route_owner_matches` — all `== 1` | Confirms the auto-generated `TokenRateLimitPolicy`'s Accepted/Enforced conditions, the MaaS gateway's Programmed condition, and the target model's `HTTPRoute` ownership all reflect real state, not just what the MaaS Setup UI happens to display. Resources found by label selector, not an assumed generated name. Scoped to internally-hosted models. |
| `rate_limit_shared_across_users` (Rate Limiting, ADR-023) | create_user (×2) → apply_rate_limit_subscription (owner_groups: [], owner_users_from_shared_state) → provision_keys_for_users → send_requests (key_index: 0) → send_requests_as_second_user (key_index: 1) → [cleanup: delete both ServiceAccounts + both keys + restore/delete MaaSSubscription] | `token_limit: 200`, `token_window: "10m"` (deliberately long — must outlast the whole run so both sequential bursts land in the same window, not just each burst individually), `request_count: 20`, `concurrency: 5` | `error_rate_pct_user_a`/`_user_b` — promql pass-through, both `< 20` | Two real, distinct ServiceAccount-minted identities share ONE `MaaSSubscription`, each with its own API key. Fires user A's burst, then immediately user B's, and compares outcomes — empirically checks whether the token-rate-limit allowance is pooled across both callers or independent per-user (open question per `docs/architecture/empirical-verification-checklist.md` §5). **A FAIL on `error_rate_pct_user_b` is the empirical finding ("limits are shared"), not a harness bug** — see ADR-023. Needs `deploy/rbac-user-provisioning.yaml` applied (meaningfully more sensitive RBAC than any other scenario needs). Not yet run against a live cluster; sizing untuned. |
...
**Note on CR-based scenarios specifically**: every scenario using `kubernetes.client.CustomObjectsApi()` (`rate_limit_validation`, `access_denied_no_policy`, `rate_limit_priority_precedence`, `platform_health_check`) was, until the ADR-009 update above, broken on every real live run — the harness Job process never configured the Kubernetes client at all, so every such task failed immediately with `urllib3.exceptions.LocationValueError: No host specified`, regardless of anything else being correct. This was only caught once `platform_health_check` was actually triggered live for the first time. Fixed in `harness/main.py:_load_kube_config()` and **confirmed live**: `platform_health_check` and `rate_limit_priority_precedence` both now pass end-to-end on a real cluster. `rate_limit_validation`/`access_denied_no_policy` share the same fix but haven't been individually re-run yet — worth doing if either was previously reported as failing.
...
8. **Enforcement cross-checks specifically (ADR-018), neither yet re-verified live since the subscription-reconciliation fix (ADR-009's third Update)**: run `rate_limit_validation` with a low `token_limit` → confirm `maas_denied_by_rate_limit` goes PASSING and `oc get limitador`/the MaaS Setup Rate Limiting tab shows `limited_calls` actually incremented, not just the pre-existing harness-side assertions. Run `access_denied_no_policy` → confirm it reaches PASS (meaning requests were genuinely denied for auth reasons, `unauthorized_count > 0` and `rate_limited_count == 0`) → cross-check the MaaS Setup UI's Access Control tab shows `quota_without_access` for `maaspal-fail-closed-test` during the run → confirm cleanup leaves no orphaned `MaaSSubscription`. Both previously failed live with `400 subscription_not_ready` (provision_api_key running before the MaaS controller reconciled the just-created subscription) — confirm that's actually resolved before chasing anything else if either fails again. If either assertion instead reads PENDING/FAILING for an unexpected reason, see the load-bearing assumption flagged in ADR-018 (key identity at the gateway = subscription's owner group) before assuming the harness code is wrong. See `docs/architecture/empirical-verification-checklist.md` for the full backlog of what else is worth checking this way.
...
10. **Scenario categories (ADR-020)**: open the scenario list in the UI → confirm 6 populated category sections (Load Testing, Rate Limiting, Access Control, Metrics Validation, API Key Lifecycle, Platform Health) in that order, plus a visible, empty "Custom" section with its placeholder text → drop a scenario YAML with no `category:` field into `scenarios/` and confirm it appears under Custom without any code change.
11. **Rate-limit priority precedence (ADR-021), not yet run against a live cluster**: run `rate_limit_priority_precedence` → confirm `expected_subscription_match_count == 1` (the auto-selected key actually bound to the higher-priority subscription) → confirm via `oc get maassubscriptions -n models-as-a-service` that both `maaspal-priority-*` subscriptions are gone after the run. If the assertion instead shows a mismatch, don't assume the harness code is wrong first — re-verify live whether `system:authenticated` eligibility and pure-integer-priority-wins are both still accurate for this cluster's MaaS version (see ADR-021's Negative consequences).
12. **Platform health checks (ADR-022), not yet run against a live cluster**: run `platform_health_check` → confirm all 4 assertions PASS → cross-check by hand (`oc get tokenratelimitpolicy/gateway/httproute`) that the flags match reality → confirm it fails honestly (not PENDING) if pointed at a model/gateway/namespace combination where one of the three resources doesn't exist.
13. **Multi-user rate-limit sharing (ADR-023), not yet run against a live cluster**: apply `deploy/rbac-user-provisioning.yaml` first (review its sensitivity before applying) → run `rate_limit_shared_across_users` → confirm two `ServiceAccount`s are minted and both keys land on the one shared `MaaSSubscription` (`subscription_echo_match_count == 2`) → read whichever way `error_rate_pct_user_b` lands: PASS suggests per-user independent allowances, FAIL suggests a shared pool — **either is a valid, useful result**, treat a FAIL as the finding, not a bug report, per ADR-023 → tune `token_limit`/`request_count`/`token_window` live if user A's solo burst doesn't comfortably succeed, since the comparison is meaningless without that baseline → confirm cleanup leaves no orphaned ServiceAccounts (`oc get sa -n maaspal | grep maaspal-pool-user`) and no orphaned `MaaSSubscription` (`oc get maassubscriptions -n models-as-a-service`).
