...
MaaS:PAL needs to authenticate to the MaaS REST API (for key lifecycle operations) and to the Kubernetes API (for creating Jobs, watching pods, managing `MaaSSubscription` CRs). Two broad authentication strategies were considered:
...
The intended user of MaaS:PAL is an admin who has already configured the namespace and RBAC. The harness operates through RHOAI's user-facing APIs (not raw cluster admin operations), but it does require elevated permissions (rhoai-admin equivalent) to provision API keys and manage subscriptions.
...
- All actions are auditable under a single, clearly named service account (`maaspal-runner` or similar).

**Negative:**
- All runs execute with the same elevated SA permissions regardless of who triggered them from the UI. There is no per-user permission enforcement within MaaS:PAL.
